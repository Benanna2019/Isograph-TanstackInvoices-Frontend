...
`)(function CreateInvoiceComponent({ data }) {
  ...
  const { loadFragmentReference: loadMutation } = useImperativeReference(
    iso(`entrypoint Mutation.CreateInvoice`)
  );

  return (
    <>
      <CreateInvoiceFormWrapper
        onSubmit={(formData: CreateInvoiceInput) =>
          loadMutation({ input: formData })
        }
      >
        <select
          name="customerId"
